...
class DefaultLogger {
  ...
  async _writeMessage(transformedMessage) {
    console[transformedMessage.method](transformedMessage.message, transformedMessage.payload);
    return true;
  }
  ...
  getProfiler({options} = {}) {
    let Profiler = (options && options.Profiler) ||  this.options.Profiler;
    if(!Profiler) {
      Profiler = 'default';
    }
    if(typeof Profiler === 'string') {
      return profilers.getProfiler(Profiler);
    }
    return Profiler;
  }
